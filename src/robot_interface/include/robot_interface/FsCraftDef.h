...
public:
	...
	int StayMode = 0;					//停留方式 0停止摆动继续向前 1完全静止

	double Intermediatedwelltime = 0.0;//中间停留时间
	double Forwardratio = 0.0;//前进比例
	double Fallbackratio = 0.0;//回退比例
	bool startArcTracking = false; //拐角是否启动电弧跟踪
		//
...
public :
	...
	double Scrubinductance = 0;//刮擦电感，2026/2/24
...
public:
	...
	double Bias = 0;					//摆弧偏差
	int Dwell_type = 1;					//等待类型 0摆动停止 1机器人停止
	...
	double CenterDistance = 0;//中心前进距离
...
public:
	int Id = -1;
	int CurrentMode = 0;		//电流模式：  0电流值
	int VoltageMode = 1;		//电压模式： 0分别  1一元化
	...
	int ArcOnJobChannelNum = 0;          //JOB模式下job号
	...
	double ArcOninductance = 0;//起弧电感
	//焊接参数
	int WeldingWorkMode = 0;		//工作模式： 0直流 1脉冲 2Job模式 3断续焊 4分别模式
	double WeldingCrt_Spd = 220;		//焊接电流
	double WeldingVtg_Strth = 23;		//焊接电压
	double WeldingSpeed = 15;		//焊接速度
	int WeldJobChannelNum = 0;          //JOB模式下job号
	double Weldinductance = 0;//焊接电感
	//收弧参数
	...
	int ArcOffJobChannelNum = 0;          //JOB模式下job号
	double ArcOffinductance = 0;//收弧电感

	int	 WelderParamType = 0;									//焊机参数类型 0 一元模式     1分别模式
	int	VtgUniCorrection = 0;									//电压一元修正值
...
//弧坑回填参数
struct ArcPitBackfill {
	bool enable = false; //回填使能
	bool swingenable = false; //回填摆焊使能
	double current = 0.0; // 回填电流 (A)
	double voltage = 0.0; // 回填电压 (V)
	double voltageAdjust = 0.0; // 回填电压修正值 (%)
	double distance = 0; //回填距离(mm)
	double speed = 1;//回填速度（mm/s）
	int jobNumber = 0; // 回填JOB号
	double inductanceCorrection = 0.0; // 回填电感修正值

	int swingMode = 0;//摆动方式
	double amplitudeLeft = 0.0; // 左摆幅度 (mm)
	double amplitudeRight = 0.0; // 右摆幅度 (mm)
	double frequency = 0.0; // 摆动频率 (Hz)
	double leftStaytime = 0.0; // 左停留时间 (ms)
	double rightStaytime = 0.0; // 右停留时间 (ms)
	double leftSwingangles = 0.0;//左摆角度
	double rightSwingangle = 0.0;//右摆角度
	int stopMode = 0;//停留方式 	
};

//Cloos焊接参数
class CLOOSStartParam {
public:
	int processCurve = 1;                       // 工艺曲线：脉冲/直流等工艺类型
	double wireFeedSpeed = 20.00;               // 送丝速度：焊丝输送速度(m/min)
	double arcLength = 20.00;                   // 电弧长度：电弧长度调节参数
	double dynamicInductance = 20.00;           // 动态电感：电弧稳定性调节参数
	double variable = 20.00;                    // 变量：备用调节参数
	//引弧
	double ignitionWireSpeed = 20.00;           // 引弧送丝速度：引弧阶段焊丝输送速度
	//收弧
	double closingWireSpeed = 20.00;            // 收弧送丝速度：收弧阶段焊丝输送速度
};
